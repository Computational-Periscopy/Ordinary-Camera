...
% 3D chair occluder, real reflecting 3D scene
% ------------------------------Pseudo-code------------------------------
...
%Data path
if ismac
    calibParams.filepath = './Data/TestPosChair3DScene/';
elseif ispc
    calibParams.filepath = '.\Data\TestPosChair3DScene\';
end
...
%% Load data
[test_image1,~]=load_image1('image_test_3.mat',calibParams.filepath,downsamp_factor);   

%% Reconstruction

sr = 0.4*1.2375e+04/(Ndiscr_mon^2)*prod(subblocksperaxis); 
sg = 0.4*1.5863e+04/(Ndiscr_mon^2)*prod(subblocksperaxis); 
sb = 0.4*20625/(Ndiscr_mon^2)*prod(subblocksperaxis);
final_im1 = reconstruct_tv_it_cbg(simA,[sr,sg,sb],  test_image1, 1e08*[0.26,0.281, 1.058], NumBlocks_sim, [0,0,0]);
